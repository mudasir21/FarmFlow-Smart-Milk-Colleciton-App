...
class _ConfirmedPickupsScreenState extends State<ConfirmedPickupsScreen> {
    ...
    try {
      ...
      if (user != null) {
        ...
        if (widget.selectedDate != null) {
          ...
          // Define time ranges based on slot
          if (widget.selectedSlot != null) {
            if (widget.selectedSlot!.toLowerCase() == 'morning') {
              // Morning slot: 6 AM - 10 AM
              startTime = startOfDay.add(const Duration(hours: 6)); // 6 AM
              endTime = startOfDay.add(const Duration(hours: 10)); // 10 AM
            } else if (widget.selectedSlot!.toLowerCase() == 'evening') {
              // Evening slot: 4 PM - 7 PM
              startTime = startOfDay.add(const Duration(hours: 16)); // 4 PM
              endTime = startOfDay.add(const Duration(hours: 19)); // 7 PM
            } else {
              // If slot is neither morning nor evening, use whole day
              startTime = startOfDay;
              endTime = startOfDay.add(const Duration(days: 1));
            }
          } else {
            // If no slot specified, use whole day
            startTime = startOfDay;
            endTime = startOfDay.add(const Duration(days: 1));
          }
          ...
          query = query
              .where('pickup_date', isGreaterThanOrEqualTo: Timestamp.fromDate(startTime))
              .where('pickup_date', isLessThan: Timestamp.fromDate(endTime));
        }
  ...
  Widget build(BuildContext context) {
    ...
    String getTimeRangeDescription() {
      if (widget.selectedSlot?.toLowerCase() == 'morning') {
        return '(6 AM - 10 AM)';
      } else if (widget.selectedSlot?.toLowerCase() == 'evening') {
        return '(4 PM - 7 PM)';
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.selectedDate != null
              ? 'Pickups for ${DateFormat('MMM d').format(widget.selectedDate!)} ${getTimeRangeDescription()}'
                      ...
                      ? Center(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                widget.selectedDate != null
                                    ? 'No pickups for ${DateFormat('MMM d').format(widget.selectedDate!)} ${getTimeRangeDescription()}'
      ...
      floatingActionButton: _confirmedPickups.isEmpty
          ? null
          : FloatingActionButton.extended(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => PickupRouteNavigationScreen(
